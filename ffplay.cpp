...
static void toggle_mute_on(VideoState* is)
{
    is->muted = 1;
}

static void toggle_mute_off(VideoState* is)
{
    is->muted = 0;
}
...
static int audio_thread(void* arg)
{
    ...
    do {
        ...
        if (got_frame) {
            ...
            reconfigure =
                cmp_audio_fmts(is->audio_filter_src.fmt, is->audio_filter_src.channels, (AVSampleFormat)frame->format, frame->channels) ||
            ...
            while ((ret = av_buffersink_get_frame_flags(is->out_audio_filter, frame, 0)) >= 0) {
                ...
                AVRational av_ration = {
                    frame->nb_samples, frame->sample_rate
                };
                af->duration = av_q2d(av_ration);
...
static int audio_decode_frame(VideoState* is)
{
    ...
    if (af->frame->format != is->audio_src.fmt ||
        ...
        swr_free(&is->swr_ctx);
        is->swr_ctx = swr_alloc_set_opts(NULL, is->audio_tgt.channel_layout, is->audio_tgt.fmt, is->audio_tgt.freq, dec_channel_layout, (AVSampleFormat)af->frame->format, af->frame->sample_rate, 0, NULL);
...
static void toggle_border(VideoState* is)
{
    if (borderless) {
...
static void seek_chapter(VideoState* is, int incr)
{
    ...
    stream_seek(is, av_rescale_q(is->ic->chapters[i]->start, is->ic->chapters[i]->time_base, av_get_time_base_q()), 0, 0);
}

static SDL_HitTestResult hit_test_callback(SDL_Window* win, const SDL_Point* pt, void* data)
{
    int x, y, r_x, r_y, w_x, w_y;
    SDL_GetGlobalMouseState(&x, &y);
    SDL_GetMouseState(&r_x, &r_y);
    w_x = x - r_x;
    w_y = y - r_y;
    SDL_SetWindowPosition(win, w_x, w_y);
...
static void event_loop(VideoState* cur_stream)
{
    ...
    for (;;) {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_RETURN:
            case SDLK_KP_ENTER:
                toggle_hidden(cur_stream);
                break;
            case SDLK_COMMA:
                update_opacity(cur_stream, -0.1);
                break;
            case SDLK_PERIOD:
                update_opacity(cur_stream, 0.1);
                break;
            case SDLK_LEFTBRACKET:
                shrink_window(cur_stream);
                break;
            case SDLK_RIGHTBRACKET:
                enlarge_window(cur_stream);
                break;
            case SDLK_b:
                toggle_border(cur_stream);
                break;
            case SDLK_f:
...
{
    show_mode = !strcmp(arg, "video") ? VideoState::ShowMode::SHOW_MODE_VIDEO :
        !strcmp(arg, "waves") ? VideoState::ShowMode::SHOW_MODE_WAVES :
        !strcmp(arg, "rdft") ? VideoState::ShowMode::SHOW_MODE_RDFT :
        (VideoState::ShowMode)parse_number_or_die(opt, arg, OPT_INT, 0, VideoState::ShowMode::SHOW_MODE_NB - 1);
    return 0;
}
